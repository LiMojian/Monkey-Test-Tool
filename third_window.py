...
def open_third_window(selected_sn,opened_windows):
    ...
    def set_icon():
        ...
        third_window.iconphoto(False, icon)

    third_window = tk.Toplevel()
    third_window.after(100, set_icon)
    third_window.title("{}-在线测试".format(selected_sn))
    # 设置窗口大小
    ...
    # 创建Listbox表格
    listbox1 = tk.Listbox(third_window, selectmode=tk.MULTIPLE)
    ...
    def check_logcat():
        logcat_dir = os.path.join("logcat")  # 相对路径
        if not os.path.exists(logcat_dir):
            os.makedirs(logcat_dir)

    def check_MonkeyLog():
        MonkeyLog_dir = os.path.join("MonkeyLog")  # 相对路径
        if not os.path.exists(MonkeyLog_dir):
            os.makedirs(MonkeyLog_dir)
    ...
    def create_blacklist():
        ...
        logcat_folder = "logcat"
        logcat_file = os.path.join(logcat_folder, save_path)
        # 创建新线程执行logcat进程启动
        logcat_process = subprocess.Popen(["adb", "-s", selected_sn, "logcat"],
                                          stdout=open(logcat_file, 'w', encoding='utf-8'),
        ...
        # 创建新线程执行Monkey测试
        monkey_thread = threading.Thread(target=start_monkey_white_test, args=(logcat_process,monkey_cmd,save_path))
        monkey_thread.start()
    ...
    def create_whitelist():
        ...
        # text.insert(tk.END, "{}:Monkey执行命令{}\n".format(datetime.datetime.now(),monkey_cmd))
    ...
    validate = third_window.register(validate_input)
    validate2 = third_window.register(validate2_input)

    listbox1.bind("<<ListboxSelect>>", on_select)

    if not os.path.exists('user_settings_online.json'):
        user_settings = {
            'input1': 50,
            'input2': 10,
            'input3': 5,
        ...
        with open('user_settings_online.json', 'r') as file:
            user_settings = json.load(file)


    black_button = tk.Button(third_window, text="黑名单测试", command=create_blacklist)
    black_button.place(x=200, y=370,anchor="center")

    white_button = tk.Button(third_window, text="白名单测试", command=create_whitelist)
    white_button.place(x=600, y=370, anchor="center")

    label1 = tk.Label(third_window, text="请选择要测试的包:", font=("Arial", 15))
    label1.place(x=10, y=0)

    label2 = tk.Label(third_window, text="日志输出等级:", font=("Arial", 10))
    label2.place(x=600, y=30)
    combo_box = ttk.Combobox(third_window, values=["简略", "一般", "详细"], width=5, state="readonly")
    combo_box.set(user_settings.get('dropdown', '详细'))
    combo_box.place(x=700,y=30)

    label3 = tk.Label(third_window, text="Monkey执行次数:", font=("Arial", 10))
    label3.place(x=600, y=120)
    search_frequency = tk.Entry(third_window, validate="key", validatecommand=(validate2, "%P"))
    search_frequency.insert(0, str(user_settings['input11']))
    search_frequency.place(x=600, y=140, width=100)

    label4 = tk.Label(third_window, text="延迟时间:", font=("Arial", 10))
    label4.place(x=600, y=160)
    search_time = tk.Entry(third_window, validate="key", validatecommand=(validate2, "%P"))
    search_time.insert(0, str(user_settings['input12']))
    search_time.place(x=600, y=180, width=100)

    label5 = tk.Label(third_window, text="设置事件占比:", font=("Arial", 14))
    label5.place(x=400, y=30)


    label_touch = tk.Label(third_window, text="触摸事件:", font=("Arial", 10))
    label_touch.place(x=400, y=60)
    search_touch = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_touch.insert(0, str(user_settings['input1']))
    search_touch.place(x=500, y=60, width=50)

    label_motion = tk.Label(third_window, text="动作事件:", font=("Arial", 10))
    label_motion.place(x=400, y=90)
    search_motion = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_motion.insert(0, str(user_settings['input2']))
    search_motion.place(x=500, y=90, width=50)

    label_trackball = tk.Label(third_window, text="轨迹球事件:", font=("Arial", 10))
    label_trackball.place(x=400, y=120)
    search_trackball = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_trackball.insert(0, str(user_settings['input3']))
    search_trackball.place(x=500, y=120, width=50)

    label_nav = tk.Label(third_window, text="导航事件:", font=("Arial", 10))
    label_nav.place(x=400, y=150)
    search_nav = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_nav.insert(0, str(user_settings['input4']))
    search_nav.place(x=500, y=150, width=50)

    label_majornav = tk.Label(third_window, text="主导航事件:", font=("Arial", 10))
    label_majornav.place(x=400, y=180)
    search_majornav = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_majornav.insert(0, str(user_settings['input5']))
    search_majornav.place(x=500, y=180, width=50)

    label_syskeys = tk.Label(third_window, text="系统按键事件:", font=("Arial", 10))
    label_syskeys.place(x=400, y=210)
    search_syskeys = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_syskeys.insert(0, str(user_settings['input6']))
    search_syskeys.place(x=500, y=210, width=50)

    label_appswitch = tk.Label(third_window, text="应用切换事件:", font=("Arial", 10))
    label_appswitch.place(x=400, y=240)
    search_appswitch = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_appswitch.insert(0, str(user_settings['input7']))
    search_appswitch.place(x=500, y=240, width=50)

    label_anyevent = tk.Label(third_window, text="任意事件:", font=("Arial", 10))
    label_anyevent.place(x=400, y=270)
    search_anyevent = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_anyevent.insert(0, str(user_settings['input8']))
    search_anyevent.place(x=500, y=270, width=50)

    label_pinchzoom  = tk.Label(third_window, text="捏合缩放事件:", font=("Arial", 10))
    label_pinchzoom .place(x=400, y=300)
    search_pinchzoom = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_pinchzoom.insert(0, str(user_settings['input9']))
    search_pinchzoom.place(x=500, y=300, width=50)

    label_rotation = tk.Label(third_window, text="旋转事件:", font=("Arial", 10))
    label_rotation.place(x=400, y=330)
    search_rotation = tk.Entry(third_window, validate="key", validatecommand=(validate, "%P"))
    search_rotation.insert(0, str(user_settings['input10']))
    search_rotation.place(x=500, y=330, width=50)


    label6 = tk.Label(third_window, text="调试:", font=("Arial", 14))
    label6.place(x=600, y=210)

    check_crashes_var = tk.IntVar()
    checkbox_crashes = tk.Checkbutton(third_window, text="忽略应用程序崩溃", variable=check_crashes_var)
    check_crashes_var.set(user_settings['checkbox1'])
    checkbox_crashes.place(x=600, y=240)

    check_timeout_var = tk.IntVar()
    checkbox_timeout = tk.Checkbutton(third_window, text="忽略超时", variable=check_timeout_var)
    check_timeout_var.set(user_settings['checkbox2'])
    checkbox_timeout.place(x=600, y=270)

    check_exceptions_var = tk.IntVar()
    checkbox_exceptions = tk.Checkbutton(third_window, text="忽略安全异常", variable=check_exceptions_var)
    check_exceptions_var.set(user_settings['checkbox3'])
    checkbox_exceptions.place(x=600, y=300)

    check_native_crashes_var = tk.IntVar()
    checkbox_native_crashes = tk.Checkbutton(third_window, text="忽略本地代码崩溃崩溃", variable=check_native_crashes_var)
    check_native_crashes_var.set(user_settings['checkbox4'])
    checkbox_native_crashes.place(x=600, y=330)

    # 禁用多个输入框的双击和长按拖动事件
    for entry in [search_touch, search_motion, search_trackball, search_nav, search_majornav, search_syskeys, search_appswitch, search_anyevent, search_pinchzoom, search_rotation, search_frequency, search_time]:
        bind_entry(entry)

    scrollbar2 = tk.Scrollbar(third_window)
    scrollbar2.place(x=700, y=395, height=15)

    text = tk.Text(third_window, height=15, width=113, yscrollcommand=scrollbar2.set)
    text.place(x=1,y=395)
    text.insert(tk.END, "")

    # 将Scrollbar控件与Text控件绑定
    scrollbar2.config(command=text.yview)
    # 绑定 Ctrl+C 到 copy_text 函数
    third_window.bind('<Control-c>', copy_text)

    # 添加右键菜单
    def show_context_menu(event):
        context_menu.post(event.x_root, event.y_root)

    context_menu = tk.Menu(third_window, tearoff=0)
    ...
    def get_next_log_filename():
        # 获取当前目录下logcat文件夹中所有以数字命名的日志文件
        logcat_folder1 = os.path.join(os.getcwd(), "logcat")
        log_files = [f for f in os.listdir(logcat_folder1) if
                     os.path.isfile(os.path.join(logcat_folder1, f)) and f[:-4].isdigit() and f.endswith('.txt')]
        if log_files:
            # 获取最大的日志文件编号
            max_number = max([int(f[:-4]) for f in log_files])
            next_number = max_number + 1
        else:
            next_number = 1
        return f"{next_number}.txt"

    def capture_logcat(selected_sn, save_path):
        save_path = get_next_log_filename()
        # 执行adb命令抓取logcat并将输出重定向到文件
        text.insert(tk.END, "{}:开始抓取logcat日志".format(datetime.datetime.now()))
        command = f"adb -s {selected_sn} logcat > logcat/{save_path}"
    ...
    def start_monkey_white_test(logcat_process,monkey_cmd,save_path):
        black_button.config(state=tk.DISABLED)  # 将按钮置灰
        white_button.config(state=tk.DISABLED)  # 将按钮置灰
        global capturing_logcat
        ...
        # 稍微等待一下，确保PID获取线程已经启动
        time.sleep(1)

        try:
            # 执行Monkey测试
            output = subprocess.check_output(monkey_cmd, shell=True, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
            # Monkey测试中断，输出错误信息
            text.insert(tk.END, "{}:Monkey测试中断-{}\n".format(datetime.datetime.now(), e.output))

        text.insert(tk.END, "{}:Monkey测试完成\n".format(datetime.datetime.now()))
        # 停止抓取logcat
        capturing_logcat = False
        # 终止logcat进程
        subprocess.run(["taskkill", "/F", "/T", "/PID", str(logcat_process.pid)], shell=True, capture_output=True,
                       text=True, encoding='utf-8', errors='ignore')
        text.insert(tk.END, "{}:停止抓取logcat\n".format(datetime.datetime.now()))
        current_path = os.getcwd()
        text.insert(tk.END, "logcat日志路径为：{}\Logcat\{}\n".format(current_path, save_path))
        text.insert(tk.END, "Monkey日志路径为：{}\MonkeyLog\{}\n".format(current_path, save_path))

        black_button.config(state=tk.NORMAL)
    ...
    def save_settings():
        ...
        with open('user_settings_online.json', 'w') as file:
            json.dump(user_settings, file)

    def on_closing():
        global confirm_dialog_shown
        state = black_button.cget('state')
        if state == 'disabled'and not confirm_dialog_shown:
            confirm_dialog_shown = True
            response = tkinter.messagebox.askyesno("确认", "现在关闭可能导致正在运行的Monkey测试中断，是否真的要关闭")
            if response:
                confirm_dialog_shown = False
                save_settings()
    ...
    opened_windows[selected_sn] = third_window
    return third_window
